...
public class GraphChartFeed : MonoBehaviour
{
	void Start ()
    {
        ...
        if (graph != null)
        {
            ...
            graph.DataSource.ClearCategory("Trunk");
            graph.DataSource.ClearAndMakeBezierCurve("Player 2");
            
            for (int i = 0; i <10; i++)
            {
                graph.DataSource.AddPointToCategory("Player 1", i*5,Random.value*10f + 10f);
                graph.DataSource.AddPointToCategory("Trunk",    i*5, Random.value * 10f + 15f);
                if (i == 0)
