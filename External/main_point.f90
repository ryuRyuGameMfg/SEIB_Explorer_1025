...
!*   https://ftp.emc.ncep.noaa.gov/mmb/gcp/ldas/noahlsm/ver_2.7.1/basic/      *
!*                                                                            *
!*   Latest information and code using policy are available at                *
!*   http://seib-dgvm.com/en/                                                 *
!******************************************************************************
...
SUBROUTINE main_loop ( &
           ...
           !★ [For SEIB-Explorer]
           FileName_Stop, PathName_Out, Input_FN_Result)
...
!Location data
   ...
   !★ [For SEIB-Explorer] 
   character(len=100),intent(IN):: FileName_Stop !停止信号ファイル
   character(len=100),intent(IN):: PathName_Out  !出力ファイル書き出し先
   character(len=100),intent(IN):: Input_FN_Result   !結果出力ファイル名（拡張子無し）
   character(len=256)           :: fname         !ファイル操作用
   logical                      :: istop         !ファイル操作用
...
!Parameter variables whose initial values will be given by the parameter file
   ...
   real:: SNOMLT_sum  = 0.0 !SNOW MELT (mm/day)
...
! 10        Observed value @ Sppaskaya-pad, Siberia -Inserted by H.Sato-
   ...
   If (Flag_land_physics) then
      if (SoilClass>= 1 .and. SoilClass< 9) then 
            SOILTYP = SoilClass
      else
            SOILTYP = 5 !Default
      endif
...
!Initialize variables
   ...
   Call radiation_seasonal_change ()
...
if (Flag_output_write) then
   ...
   open ( File_no(22), file = 'output_VegStructure.txt' )
   open ( File_no(23), file = 'output_for_viewer2.txt'  )
endif

   !★ [For SEIB-Explorer] 
   open ( File_no(23), file = trim(Input_FN_Result)//'.txt'  )
   
   
!_____________ NOAH-LSM Integration (Phase 5: Compute daily chages of radiation)
IF (Flag_land_physics) THEN
   Call radiation_daily_change (TimeStepMax, RadShortWeight)
END IF
...
!TMP★
write (*,*) 'counter_begin & counter_end', counter_begin, counter_end
...
!_____________ DAILY PSYSICAL PROCESSES
   ...
   Call radiation (rad_short_Today, cloud)
...
!For preparing output variables
   ...
   SNOMLT_sum  = SNOMLT_sum + SNOMLT * 1000    !SNOW MELT (mm)
...
!This paragraph contains all procedures for providing variables from NOAH-LSM to SEIB-DGVM
   ...
   if (SFCTMP <= ABS_ZERO) then !Air temperature (K) <= 273.15
      flux_sn_RunningRecord (1) = prec_Today
   else
      flux_sn_RunningRecord (1) = 0.0
   endif
...
if (Flag_photosynthesis_type) then
   ...
   Call leaf_season (tmp_soil_Today )
...
!_____________ MAKE OUTPUT FILES (Write simulation results in output files)
   ...
   !★ [For SEIB-Explorer] 
   Call output_for_viewer2 &
   (File_no(23), ALT, Albedo_soil0, W_fi, W_wilt, W_sat, &
   tmp_air_Today, &
...
IF (Flag_output_write) then
   ...
   (File_no(2), ALT, Albedo_soil0, W_fi, W_wilt, W_sat, &
   tmp_air_Today, &
   sum(tmp_soil_Today(1:5))/5.0, sum(tmp_soil_Today(6:10))/5.0, sum(tmp_soil_Today(11:20))/10.0, &
   prec_Today, rh_Today, cloud, rad_short_Today, rad_long_Today, wind_Today)
   ...
   !Daily output (during the last 10 years)
   If ( year>Simulation_year-10) then
      Call output_climate &
...
!  if (doy == 213) then !@ 1 Aug
      write(*,*) 'flag_loc4', year !TMP
      Call output_annual       (File_no(13))
      write(*,*) 'flag_loc5', year !TMP
      Call output_forest       (File_no(14))
      write(*,*) 'flag_loc6', year !TMP
      Call output_BasalArea    (File_no(20))
      write(*,*) 'flag_loc7', year !TMP
      Call output_DBH_hist     (File_no(21))
      write(*,*) 'flag_loc8', year !TMP
      Call output_VegStructure (File_no(22), ALT, Albedo_soil0, W_fi, W_wilt, W_sat)
      write(*,*) 'flag_loc9', year !TMP
   endif
...
if ( doy==Day_in_Year ) then
   ...
   open(newunit=i, file=fname, status="replace", action="write") !, iostat=ios)
   !if (ios /= 0) then
   !   print *, "[Fortran] ファイル出力失敗: ", fname
   !   return
   !end if
   
   write(i, '(A)') "This file is only for time counter" !ヘッダー
   !write(u, '(I0,",",F6.2,",",F5.2,",",F5.2,",",F6.2,",",F6.2)') &
   !    year, npp, lai, temp, precip, co2
   
   close(i)
   
   !毎年、中断の指令が入っていないか確認する
...
if (Flag_output_write) then
   ...
   close (File_no(22))   !output_VegStructure.txt ![Newly introduced at version 3.12]
   close (File_no(23))   !output_for_viewer2.txt ![Newly introduced at version 3.12]
endif
...
!**************************************************************************************************
SUBROUTINE radiation_daily_change (TimeStepMax, RadShortWeight)
   USE data_structure
   USE grid_status_current2
   implicit none
