...
SUBROUTINE output_for_viewer &
   (Fn, ALT, Albedo_soil0, W_fi, W_wilt, W_sat, &
...
!Local variables
   ...
   integer i, j, p, no, type_grass, count
   real    a1, a2, a3, a4, a5, a6, b1, b2, b3, c1, c2, x
   real    tree_density, Dist_step
...
If ( counter==1 .or.  (Flag_spinup_read .and. counter==1+Spinup_year*Day_in_Year) ) then
   ...
   !File identifier
   write (Fn, '(a18)')    'output_for_viewer2'
   write (Fn, '(i4, a1, i4)') Max_loc, ',', DivedG
   write (Fn, '(i5,a,i5,a,i4)')  Simulation_year,',',PFT_no,',',NumSoil
   write (Fn, '( 3(f7.2,a), 3(f7.5,a), f7.5 )' )  &
          LAT,',',LON,',',ALT,',',Albedo_soil0,',',W_fi,',',W_wilt,',',W_sat
...
If ( doy==1 ) then
   ...
   write (Fn,'(f7.1)' ) CO2atm !Atmospheric CO2 (ppm)
   !write (Fn,*)
Endif
...
!_____________ Write Annual output (Annual maximum LAI of grass layer)
IF ( Doy==Day_in_Year ) then
   Do j=1, DivedG
      do i=1, DivedG
         write (Fn,'(f4.1,a1)', advance='no') lai_grass_max(i,j), ','
      end do
      write (Fn,*) !Line feed
   End do
END IF
...
END SUBROUTINE output_for_viewer2
...
!**************************************************************************************************
! ★Output Annual changes in Vegetation Structure
!**************************************************************************************************
SUBROUTINE output_VegStructure (Fn, ALT, Albedo_soil0, W_fi, W_wilt, W_sat)
...
END SUBROUTINE output_VegStructure
...
!_________________ Write data
   write (Fn,'( i3 , i4 , i3 , a3, 5f8.1, 2f6.2 )') &
