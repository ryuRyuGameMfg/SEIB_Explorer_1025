...
!              accroding to their PFT and mean daytime PAR
   ...
   DO no=1, Max_no
     ...
     !psat  (no) = psat_today  (p,i)                                   !Disabled 2021/April/15
      psat  (no) = psat_today  (p,i) * (pmax_acclimated(no) / max(1.0,Pmax(p))) !Mod 2021/April/15
...
!**************************************************************************************************
SUBROUTINE leaf_season (tmp_soil)
...
!Local parameters for tropical rain forest from Huth & Ditzer (2000)
   ...
   real,parameter::LAgrow_max_day = 0.5
...
IF ( .not. phenology(pft(no)) ) cycle
    ...
    a1 = 0.5 * (dbh_heartwood(no) + dbh_sapwood(no))    !Trunk radius at 1.3m height [m]
    a2 = a1 * a1 * PI                                   !Cross Selction Area at 1.3m height [m2]
    a3 = a2 * (height(no)-bole(no)) / max(1,height(no)) !Cross Selction Area at Crown Base [m2]
    
    !Constrain based on Equation 3b (GLMM) of Sumida et al. (2023) Sci. Rep.
    a4 = 1.077 * 2742.0 * (a3**0.955)                   !LAmax [m2]
    a1 = a4 / SLA(p)                                    !LAmax [gDM]
    a1 = max(0.0, a1-mass_leaf(no) )
    
    !a2: maximum foliage mass with current resource available (g)
    a2 = mass_available(no) / RG_f(p)
...
!Local variables
   ...
   integer p            !PFT number
   integer no,i,j       !for general usage
...
if ( height(no)-bole(no) <= Crown_depth_min ) cycle
   ...
   Case (7,8)     !Cold temperate PFTs (Tatsumi et al. 2012 FORMATH vol11)
      !年間NPPが悪い樹冠レイヤーの枯れ上げ
      do j = 1, min(10, height(no)-bole(no)-1)
        if ( sum(npp_crownbottom(no,1:j))/j < npp_crowntop(no)*ALM_crown2(p) ) count=max(count,j)
      end do
...
!Local variables
   ...
   real    x, y           !for general usage
   real    a1, a2, a3, a4, a5 !for general usage
   integer no, p, count   !
...
!     x = PI * ((46.4587*diameter_new)**2)
   ...
   elseif (Life_type(p)==7 .or. Life_type(p)==8)    then
      ...
      !樹冠経は樹冠直下の幹断面積によっても制約される
      
      a1 = real(height(no)-bole(no)) * STEP            !a1: 樹冠長(m)
      a2 = real(height(no)) * STEP + 1.3               !a2: 樹高(m)
      a3 = (dbh_heartwood(no) + dbh_sapwood(no)) / 2.0 !a3: DBHにおける幹断面積 (m2)
      a3 = 3.14 * a3 * a3
      a4 = a3 * a1 / max(a2-1.3, 0.001)                !a4: 樹冠直下における幹断面積 (m2)
      a4 = max(a4, 0.001)
      if (p==10) then; a5 = 8.3389 * (a4 ** 0.2929)    !a5: 樹冠半径 (m) for 冷温帯針葉樹PFTs
      else           ; a5 = 12.218 * (a4 ** 0.3028)    !a5: 樹冠半径 (m) for 冷温帯広葉樹PFTs
      endif
      x = PI * a5 * a5
