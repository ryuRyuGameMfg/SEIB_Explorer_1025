...
!est_capacity(1:PFT_no): establishment potential for each PFT based on climatic range
   ...
   do p = 1, PFT_no
      ...
      ![OPTION] For reducing the sharpnesses of the climatic envelopes
      !         all climatic limitations are multiplied by a random factor 
      !         that uniformly distributes 0.9-1.1 .
      y = 1.0 + 0.2 * (randf() - 0.5) !y has a uniform distribution at 0.9-1.1
      !y = 1.0 !activate for turing off of this function
...
!      if ( Life_type(p)==2 .and. dfl_fire>50*365      ) then; est_capacity(p)=.false. ;endif
      ...
      elseif (GlobalZone==2) then
        ...
        !Default
        ! Disabled on 13 Jan 2023
        ! This is to enable the establishment of TrBR outside the African continent.
...
!TMP These PFTs are not yet implemented
est_capacity(17:18) = .false. 
